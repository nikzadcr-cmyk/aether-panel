...
__name(cgnatSubnet, "cgnatSubnet");
...
__name(readAddress, "readAddress");
...
function parseVmess(buf) {
  ...
  off2 += 1;
  off2 += 1;
  const port = buf[off2] << 8 | buf[off2 + 1];
  off2 += 2;
  const atyp = buf[off2];
  off2 += 1;
  let host = "";
  ...
  if (atyp === 1) {
    type = "ipv4";
    host = `${buf[off2]}.${buf[off2 + 1]}.${buf[off2 + 2]}.${buf[off2 + 3]}`;
    off2 += 4;
  } else if (atyp === 3) {
    type = "domain";
    const len = buf[off2];
    off2 += 1;
    host = new TextDecoder().decode(buf.subarray(off2, off2 + len));
    off2 += len;
  } else if (atyp === 4) {
    type = "ipv6";
    const parts = [];
    for (let i = 0; i < 16; i += 2)
      parts.push((buf[off2 + i] << 8 | buf[off2 + i + 1]).toString(16));
    host = parts.join(":");
    off2 += 16;
  } else {
    throw new Error(`vmess: unsupported ATYP 0x${atyp.toString(16)}`);
  }
  const idBytes = buf.subarray(off2, off2 + 16);
  const hex = Array.from(idBytes).map((b) => b.toString(16).padStart(2, "0")).join("");
  const uuid = `${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20, 32)}`;
  off2 += 16;
  const payload = buf.subarray(off2);
  return { protocol: "vmess", uuid, target: { host, port, type }, payload: payload.slice(), raw: buf.slice() };
}
__name(parseVmess, "parseVmess");
...
__name(parseFirstFrame, "parseFirstFrame");
...
__name(connectProxy, "connectProxy");
...
__name(connectSocks5, "connectSocks5");
...
__name(connectSocks4, "connectSocks4");
...
__name(connectHttp, "connectHttp");
...
__name(socketToUpstream, "socketToUpstream");
...
__name(buildSocksAddress, "buildSocksAddress");
...
__name(readSocksReply, "readSocksReply");
...
__name(normalizeProxy, "normalizeProxy");
...
function pump(client, upstream, opts = {}) {
  ...
  const reportUp = /* @__PURE__ */ __name((n) => {
    try {
      opts.onUp?.(n);
    } catch {
    }
  }, "reportUp");
  const reportDown = /* @__PURE__ */ __name((n) => {
    try {
      opts.onDown?.(n);
    } catch {
    }
  }, "reportDown");
  const shutdown = /* @__PURE__ */ __name((err) => {
    if (closed)
      return;
    closed = true;
    if (flushTimer)
  ...
  const flushNow = /* @__PURE__ */ __name(async () => {
    if (pendingBytes === 0)
      return;
    if (writerLock) {
      await writerLock;
  ...
  const onMessage = /* @__PURE__ */ __name(async (ev) => {
    if (closed)
      return;
...
function sha224Sync(message) {
  ...
  for (let off2 = 0; off2 < padded.length; off2 += 64) {
    const W = new Uint32Array(64);
    for (let i = 0; i < 16; i++)
      W[i] = dv.getUint32(off2 + i * 4);
    for (let i = 16; i < 64; i++) {
...
async function totpAt(key, counter) {
  ...
  const off2 = sig[sig.length - 1] & 15;
  const bin = (sig[off2] & 127) << 24 | (sig[off2 + 1] & 255) << 16 | (sig[off2 + 2] & 255) << 8 | sig[off2 + 3] & 255;
...
var CACHE_MAX = 2048;
async function dohQuery(domain2, recordType = "A", dohBase = "https://cloudflare-dns.com/dns-query") {
  const key = `${domain2}:${recordType}:${dohBase}`;
  ...
  try {
    ...
    const qname = encodeName(domain2.endsWith(".") ? domain2.slice(0, -1) : domain2);
...
function parseAnswer(buf) {
  ...
  for (let i = 0; i < qdcount; i++) {
    const { end } = parseName(buf, off2);
    off2 = end + 4;
  }
  const out = [];
  for (let i = 0; i < ancount && off2 < buf.byteLength; i++) {
    const { end } = parseName(buf, off2);
    off2 = end;
    const type = dv.getUint16(off2);
    off2 += 2;
    off2 += 2;
    const ttl = dv.getUint32(off2);
    off2 += 4;
    const rdlength = dv.getUint16(off2);
    off2 += 2;
    const dataStart = off2;
    let data = "";
    if (type === 1 && rdlength === 4) {
      data = `${buf[off2]}.${buf[off2 + 1]}.${buf[off2 + 2]}.${buf[off2 + 3]}`;
      out.push({ type: "A", data, ttl });
    } else if (type === 28 && rdlength === 16) {
      const parts = [];
      for (let j = 0; j < 16; j += 2)
        parts.push((buf[off2 + j] << 8 | buf[off2 + j + 1]).toString(16));
      data = parts.join(":");
      out.push({ type: "AAAA", data, ttl });
    } else if (type === 5) {
      const { name } = parseName(buf, off2);
      data = name;
      out.push({ type: "CNAME", data, ttl });
    }
    off2 = dataStart + rdlength;
  }
  return out;
}
__name(parseAnswer, "parseAnswer");
var BLOCK_IPS = /* @__PURE__ */ new Set(["0.0.0.0", "::", "176.103.130.130", "176.103.130.131"]);
async function isDomainBlocked(domain2, cfg) {
  if (!domain2)
    return false;
  ...
  const a = await dohQuery(domain2, "A", url);
  if (a.some((r) => BLOCK_IPS.has(r.data)))
    return true;
  if (cfg.porn || cfg.ads || cfg.malware) {
    const aaaa = await dohQuery(domain2, "AAAA", url);
    if (aaaa.some((r) => BLOCK_IPS.has(r.data)))
      return true;
  }
  return false;
}
__name(isDomainBlocked, "isDomainBlocked");

// src/core/tunnel.ts
async function handleTunnel(request, env2, ctx) {
  const url = new URL(request.url);
  ...
  ctx.waitUntil(
    (async () => {
      try {
        await runSession(server, request, env2, allowed, identifierFromPath);
      } catch (e) {
        try {
...
async function runSession(server, request, env2, allowed, identifierFromPath) {
  ...
  if (parsed.protocol === "trojan") {
    ...
    user = await env2.DB.prepare("SELECT * FROM users WHERE uuid = ? OR username = ? COLLATE NOCASE LIMIT 1").bind(id, id).first().then((r) => r ?? void 0);
  }
  if (!user) {
  ...
  const doId = env2.USER_STATE.idFromName(user.username);
  const doStub = env2.USER_STATE.get(doId);
  ...
  let upstream;
  try {
    const proxy = await selectUpstream(user, env2);
    if (proxy) {
    ...
    try {
      const host = pickFallbackHost(env2);
      if (host) {
  ...
  await env2.DB.prepare("UPDATE users SET used_req = used_req + 1, last_active = ? WHERE username = ?").bind(Math.floor(Date.now() / 1e3), user.username).run().catch(() => {
  });
...
async function selectUpstream(user, env2) {
  if (user.user_socks5) {
    try {
  ...
  if (user.user_proxy_iata) {
    try {
      const id = env2.POOL_STATE.idFromName("global");
      const stub = env2.POOL_STATE.get(id);
...
__name(selectUpstream, "selectUpstream");
function pickFallbackHost(env2) {
  const list = (env2.PROXY_FALLBACK_HOSTS || "").split(",").map((s) => s.trim()).filter(Boolean);
  if (!list.length)
    return null;
  const iata = list[Math.floor(Math.random() * list.length)];
  return `${iata}.proxyip.cmliussss.net`;
}
__name(pickFallbackHost, "pickFallbackHost");
...
async function requireAuth(c, next) {
  ...
  if (auth && auth.toLowerCase().startsWith("bearer ")) {
    ...
    const token = await env2.DB.prepare(
    ...
    if (token) {
      ...
      await env2.DB.prepare("UPDATE api_tokens SET last_used = ? WHERE id = ?").bind(Math.floor(Date.now() / 1e3), token.id).run().catch(() => {
      });
      return next();
    }
  }
...
async function issueSession(c, adminId, env2) {
...
async function destroySession(c, env2) {
  const cookie = c.req.header("cookie") || "";
  const m = cookie.split(";").map((x) => x.trim()).find((x) => x.startsWith(SESSION_COOKIE + "="));
  if (m) {
    const raw2 = m.split("=").slice(1).join("=");
    const hash = await sha256Hex(raw2);
    await env2.DB.prepare("DELETE FROM sessions WHERE token_hash = ?").bind(hash).run().catch(() => {
    });
  }
...
authRoutes.post("/setup", async (c) => {
  const env2 = c.env;
  const existing = await env2.DB.prepare("SELECT COUNT(*) AS n FROM admins").first();
  ...
  await env2.DB.prepare("INSERT INTO admins (username, password_hash, role, is_active) VALUES (?,?, 'owner', 1)").bind(body.username, hash).run();
  return c.json({ ok: true });
});
authRoutes.post("/auto-bootstrap", async (c) => {
  const env2 = c.env;
  if (!env2.ADMIN_BOOTSTRAP_PASSWORD)
    return c.json({ error: "no bootstrap secret" }, 400);
  const existing = await env2.DB.prepare("SELECT COUNT(*) AS n FROM admins").first();
  if (existing && existing.n > 0)
    return c.json({ ok: true, already: true });
  const hash = await hashPassword(env2.ADMIN_BOOTSTRAP_PASSWORD);
  await env2.DB.prepare("INSERT INTO admins (username, password_hash, role, is_active) VALUES (?, ?, 'owner', 1)").bind("admin", hash).run();
  return c.json({ ok: true });
});
authRoutes.post("/login", async (c) => {
  const env2 = c.env;
  const body = await c.req.json();
  const row = await env2.DB.prepare("SELECT * FROM admins WHERE username = ? AND is_active = 1").bind(body.username).first();
  ...
  if (!row.password_hash.startsWith("pbkdf2$")) {
    ...
    await env2.DB.prepare("UPDATE admins SET password_hash = ? WHERE id = ?").bind(newHash, row.id).run();
  }
  await env2.DB.prepare("UPDATE admins SET last_login = ? WHERE id = ?").bind(nowSec(), row.id).run();
  await issueSession(c, row.id, env2);
  return c.json({ ok: true });
});
...
authRoutes.post("/change-password", requireAuth, async (c) => {
  ...
  const row = await env2.DB.prepare("SELECT password_hash FROM admins WHERE id = ?").bind(adminId).first();
  if (!row || !await verifyPassword(body.current, row.password_hash))
    return c.json({ error: "bad current password" }, 400);
  await env2.DB.prepare("UPDATE admins SET password_hash = ? WHERE id = ?").bind(await hashPassword(body.next), adminId).run();
  return c.json({ ok: true });
});
...
proxyRoutes.get("/", async (c) => {
  ...
  const args = country ? [country, pageSize, off2] : [pageSize, off2];
  const rows = await c.env.DB.prepare(
...
__name(parseProxyList, "parseProxyList");
async function syncPool(env2) {
...
systemRoutes.get("/backup", requireRole("owner", "admin"), async (c) => {
  ...
  const [users, settings, proxies, admins] = await Promise.all([
    env2.DB.prepare("SELECT * FROM users").all(),
    env2.DB.prepare("SELECT * FROM settings").all(),
    env2.DB.prepare("SELECT * FROM proxies").all(),
    env2.DB.prepare("SELECT id, username, role, is_active, created_at FROM admins").all()
  ]);
  const dump = {
    version: 1,
    exportedAt: nowSec(),
  ...
  try {
    await env2.BUCKET?.put?.(`backup-${nowSec()}.json`, JSON.stringify(dump), {
...
systemRoutes.post("/restore", requireRole("owner"), async (c) => {
  ...
  if (Array.isArray(body.settings)) {
    const stmts = body.settings.map(
      (s) => env2.DB.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, ?)").bind(s.key, s.value, nowSec())
    );
    await env2.DB.batch(stmts);
  }
...
async function ensureD1(api, headers, accountId) {
...
async function ensureKv(api, headers, accountId) {
...
__name(ensureKv, "ensureKv");
async function ensureQueue(api, headers, accountId) {
  const list = await fetch(api + "/accounts/" + accountId + "/queues", { headers });
  const lj = await list.json();
  if ((lj.result || []).some((q) => q.queue_name === "aether-writes"))
    return;
...
__name(ensureQueue, "ensureQueue");
async function applyD1Schema(api, headers, accountId, d1Id) {
...
__name(applyD1Schema, "applyD1Schema");
...
__name(randomReadablePassword, "randomReadablePassword");

// src/telegram/bot.ts
async function handleTelegramUpdate(req, env2) {
  if (!env2.TELEGRAM_TOKEN)
    return new Response("bot disabled", { status: 404 });
  const update = await req.json();
  try {
    if (update.callback_query)
      return handleCallback(update.callback_query, env2);
    if (update.message)
      return handleMessage(update.message, env2);
  } catch (e) {
...
async function handleCallback(cb, env2) {
  const chatId = cb.message?.chat.id;
  if (!chatId)
    return new Response("ok");
  await answerCb(env2, cb.id);
  if (cb.data === "build:start") {
    await setState(env2, chatId, { step: "awaiting_token", attempts: 0 });
    await sendMessage(env2, chatId, [
      "\u{1FA90} <b>\u0633\u0627\u062E\u062A \u067E\u0646\u0644 \u0627\u062E\u062A\u0635\u0627\u0635\u06CC Aether</b>",
      "",
...
async function handleMessage(msg, env2) {
  const chatId = msg.chat.id;
  const text = (msg.text || "").trim();
  const fromId = msg.from?.id;
  if (fromId && String(fromId) === String(env2.TELEGRAM_ADMIN_ID || "")) {
    if (text === "/list" || text === "/users") {
      ...
      const lines = (results || []).map((u) => {
        const r = u;
        return (r.is_active ? "\u{1F7E2} " : "\u{1F534} ") + r.username + " \u2014 " + r.used_gb.toFixed(2) + "GB / " + (r.expiry_days ?? "\u221E") + "d";
      });
      await sendMessage(env2, chatId, lines.length ? lines.join("\n") : "\u06A9\u0627\u0631\u0628\u0631\u06CC \u0646\u06CC\u0633\u062A.");
      return new Response("ok");
    }
  }
  if (text === "/start") {
    await sendStart(env2, chatId);
    return new Response("ok");
  }
  if (text === "/status") {
    await sendMessage(env2, chatId, "\u2705 \u0631\u0628\u0627\u062A \u0622\u0646\u0644\u0627\u06CC\u0646 \u0627\u0633\u062A.\n\u0646\u0633\u062E\u0647: " + env2.APP_VERSION);
    return new Response("ok");
  }
  if (text === "/cancel" || text === "/stop") {
    await clearState(env2, chatId);
    await sendMessage(env2, chatId, "\u0644\u063A\u0648 \u0634\u062F.");
    return new Response("ok");
  }
  const state = await getState(env2, chatId);
  if (state?.step === "awaiting_token") {
    if (!/^[A-Za-z0-9_\-]{30,}$/.test(text)) {
      await sendMessage(env2, chatId, "\u274C \u062A\u0648\u06A9\u0646 \u0646\u0627\u0645\u0639\u062A\u0628\u0631 \u0628\u0647 \u0646\u0638\u0631 \u0645\u06CC\u200C\u0631\u0633\u062F. \u062F\u0648\u0628\u0627\u0631\u0647 \u0628\u0641\u0631\u0633\u062A \u06CC\u0627 /cancel \u0628\u0632\u0646.");
      return new Response("ok");
    }
    const attempts = (state.attempts || 0) + 1;
    if (attempts > 3) {
      await clearState(env2, chatId);
      await sendMessage(env2, chatId, "\u062A\u0644\u0627\u0634\u200C\u0647\u0627\u06CC \u0646\u0627\u0645\u0648\u0641\u0642 \u0632\u06CC\u0627\u062F \u0628\u0648\u062F. \u0628\u0631\u0627\u06CC \u0634\u0631\u0648\u0639 \u062F\u0648\u0628\u0627\u0631\u0647 /start \u0628\u0632\u0646.");
      return new Response("ok");
    }
    await setState(env2, chatId, { ...state, attempts, last: Date.now() });
    const statusMsg = await sendMessage(env2, chatId, "\u{1F511} \u062A\u0648\u06A9\u0646 \u062F\u0631\u06CC\u0627\u0641\u062A \u0634\u062F. \u062F\u0631 \u062D\u0627\u0644 \u0633\u0627\u062E\u062A \u067E\u0646\u0644 \u0631\u0648\u06CC \u062D\u0633\u0627\u0628 \u06A9\u0644\u0648\u062F\u0641\u0644\u0631\u062A...\n(\u0627\u06CC\u0646 \u06A9\u0627\u0631 \u062D\u062F\u0648\u062F \u06F3\u06F0 \u062B\u0627\u0646\u06CC\u0647 \u0637\u0648\u0644 \u0645\u06CC\u200C\u06A9\u0634\u062F)");
    try {
      ...
      const text2 = [
        ...
        "",
        "\u{1F517} <b>\u067E\u0646\u0644:</b> " + result.url + "/panel",
        "\u{1F464} <b>\u0646\u0627\u0645 \u06A9\u0627\u0631\u0628\u0631\u06CC:</b> <code>" + result.adminUser + "</code>",
        "\u{1F511} <b>\u0631\u0645\u0632 \u0639\u0628\u0648\u0631:</b> <code>" + result.adminPassword + "</code>",
        "",
        "\u{1F4F2} <b>\u0644\u06CC\u0646\u06A9 \u0627\u0634\u062A\u0631\u0627\u06A9:</b> " + result.url + "/sub/<b>test</b>",
        "   (\u06CC\u06A9 \u06A9\u0627\u0631\u0628\u0631 test \u0628\u0627 \u06F5\u06F0GB \u0648 \u06F3\u06F6\u06F5 \u0631\u0648\u0632 \u0628\u0647\u200C\u0635\u0648\u0631\u062A \u062E\u0648\u062F\u06A9\u0627\u0631 \u0633\u0627\u062E\u062A\u0647 \u0646\u0634\u062F\u0647\u061B \u0628\u0639\u062F \u0627\u0632 \u0648\u0631\u0648\u062F \u0628\u0647 \u067E\u0646\u0644 \u06A9\u0627\u0631\u0628\u0631 \u062F\u0644\u062E\u0648\u0627\u0647\u062A \u0631\u0627 \u0628\u0633\u0627\u0632.)",
        "",
        "\u{1F6E1} <b>\u062A\u0648\u0635\u06CC\u0647 \u0627\u0645\u0646\u06CC\u062A\u06CC:</b> \u0628\u0639\u062F \u0627\u0632 \u0627\u0648\u0644\u06CC\u0646 \u0648\u0631\u0648\u062F\u060C \u0631\u0645\u0632 \u0631\u0627 \u0639\u0648\u0636 \u06A9\u0646 \u0648 \u062A\u0648\u06A9\u0646 \u06A9\u0644\u0648\u062F\u0641\u0644\u0631 \u0631\u0627 Revoke \u06A9\u0646.",
        "",
        "\u0628\u0631\u0627\u06CC \u0633\u0627\u062E\u062A \u067E\u0646\u0644 \u062F\u06CC\u06AF\u0631 /start \u0628\u0632\u0646."
      ].join("\n");
      if (statusMsg)
        await editMessage(env2, chatId, statusMsg, text2);
      else
        await sendMessage(env2, chatId, text2);
    } catch (e) {
      const errMsg = e.message || "\u062E\u0637\u0627\u06CC \u0646\u0627\u0634\u0646\u0627\u062E\u062A\u0647";
      await setState(env2, chatId, { ...state, attempts, last: Date.now() });
      if (statusMsg)
        await editMessage(env2, chatId, statusMsg, "\u274C \u062E\u0637\u0627: " + escapeHtml(errMsg) + "\n\n\u062A\u0648\u06A9\u0646 \u0631\u0627 \u0686\u06A9 \u06A9\u0646 \u0648 \u062F\u0648\u0628\u0627\u0631\u0647 \u0628\u0641\u0631\u0633\u062A\u060C \u06CC\u0627 /cancel \u0628\u0632\u0646.");
      else
        await sendMessage(env2, chatId, "\u274C \u062E\u0637\u0627: " + errMsg);
    }
    return new Response("ok");
  }
  return sendStart(env2, chatId);
}
__name(handleMessage, "handleMessage");
async function sendStart(env2, chatId) {
  await sendMessage(env2, chatId, [
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: "\u{1FA90} \u0633\u0627\u062E\u062A \u067E\u0646\u0644 \u0627\u062E\u062A\u0635\u0627\u0635\u06CC \u0645\u0646", callback_data: "build:start" }]
      ]
    }
  });
  return new Response("ok");
}
__name(sendStart, "sendStart");
function stateKey(chatId) {
  return "tgstate:" + chatId;
}
__name(stateKey, "stateKey");
async function getState(env2, chatId) {
  try {
    const v = await env2.KV.get(stateKey(chatId));
...
async function setState(env2, chatId, state) {
  await env2.KV.put(stateKey(chatId), JSON.stringify(state), { expirationTtl: 30 * 60 });
}
__name(setState, "setState");
async function clearState(env2, chatId) {
  await env2.KV.delete(stateKey(chatId));
}
...
async function sendMessage(env2, chatId, text, extra = {}) {
  ...
  const r = await fetch("https://api.telegram.org/bot" + env2.TELEGRAM_TOKEN + "/sendMessage", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(body)
  });
...
async function editMessage(env2, chatId, messageId, text) {
  await fetch("https://api.telegram.org/bot" + env2.TELEGRAM_TOKEN + "/editMessageText", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      chat_id: chatId,
...
async function answerCb(env2, id) {
  await fetch("https://api.telegram.org/bot" + env2.TELEGRAM_TOKEN + "/answerCallbackQuery", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
function panelHtml(version2, bootstrap = false) {
  return `<!doctype html>
  ...
  <aside id="sidebar" class="sidebar w-64 flex-shrink-0 p-4 flex flex-col" style="position:sticky;top:0;height:100vh">
    <div class="flex items-center gap-3 px-2 py-3 mb-6">
      ...
      <div>
        ...
        <div class="text-[10px] text-slate-500 font-mono">v${version2}</div>
      </div>
    </div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        ...
        <div class="glass rounded-2xl p-5 md:col-span-2">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 text-sm">
            <div class="p-3 rounded-xl" style="background:rgba(148,163,184,.05)"><div class="text-xs text-slate-400">\u0646\u0633\u062E\u0647</div><div class="font-mono mt-1">${version2}</div></div>
...
function buildLinks(user, ctx) {
  ...
  const tlsParam = security === "tls" ? "&security=tls&sni=" + encodeURIComponent(sni) + "&fp=" + fp + "&alpn=" + alpnList.join(",") + "&allowInsecure=" + (user.allow_insecure ? 1 : 0) : "&security=none";
  ...
  if (connType.includes("vless")) {
    out.push(
      "vless://" + user.uuid + "@" + host + ":" + port + "?encryption=none&security=" + security + "&sni=" + encodeURIComponent(sni) + "&fp=" + fp + "&alpn=" + alpnList.join(",") + "&allowInsecure=" + (user.allow_insecure ? 1 : 0) + "&type=ws&host=" + encHost + "&path=" + encPath + frag + "#" + encodeURIComponent(label + "|vless")
    );
  }
  if (connType.includes("trojan")) {
    out.push(
      "trojan://" + user.uuid + "@" + host + ":" + port + "?security=" + security + "&sni=" + encodeURIComponent(sni) + "&fp=" + fp + "&alpn=" + alpnList.join(",") + "&allowInsecure=" + (user.allow_insecure ? 1 : 0) + "&type=ws&host=" + encHost + "&path=" + encPath + frag + "#" + encodeURIComponent(label + "|trojan")
    );
  }
...
function buildClash(user, ctx, links) {
  ...
  const directRules = directDomains.length ? directDomains.map((d) => `  - DOMAIN-SUFFIX,${d},DIRECT`).join("\n") : "";
  const blockRules = blockDomains.length ? blockDomains.map((d) => `  - DOMAIN-SUFFIX,${d},REJECT`).join("\n") : "";
...
__name(buildClash, "buildClash");
function buildSingBox(_user, _ctx, _links) {
  ...
  for (const d of directDomains)
    rules.push({ domain_suffix: [d], outbound: "direct" });
  for (const d of blockDomains)
    rules.push({ domain_suffix: [d], outbound: "block" });
...
__name(buildSingBox, "buildSingBox");
...
__name(ctxWaitUntil, "ctxWaitUntil");
...
var rawFetch = /* @__PURE__ */ __name(async (request, env2, ctx) => {
  ctx.waitUntil(ensurePoolSynced(env2));
  const url = new URL(request.url);
  const upgrade = request.headers.get("upgrade") || "";
  if (upgrade.toLowerCase() === "websocket") {
    return handleTunnel(request, env2, ctx);
  }
...
var scheduled = /* @__PURE__ */ __name(async (event, env2, ctx) => {
  ...
  if (event.cron === "* * * * *") {
    const id = env2.POOL_STATE.idFromName("global");
    ctx.waitUntil(env2.POOL_STATE.get(id).fetch("http://do/health-check"));
  }
  if (event.cron === "*/5 * * * *") {
    ctx.waitUntil(autoRotateIps(env2));
  }
  if (event.cron === "0 * * * *") {
    ctx.waitUntil(autoResetQuotas(env2));
  }
}, "scheduled");
var queue = /* @__PURE__ */ __name(async (batch, env2) => {
...
async function autoRotateIps(env2) {
  try {
    const now = Date.now();
    const { results } = await env2.DB.prepare(
    ...
    if (!results?.length)
      return;
    const res = await fetch(env2.PRIMARY_FETCH).catch(() => null);
    if (!res || !res.ok)
      return;
    const text = await res.text();
    ...
    for (const u of results) {
      ...
      if (!pool.length)
        continue;
      const count3 = Number(u.ip_count) || 15;
      const chosen = [];
      for (let i = 0; i < count3 && pool.length; i++)
        chosen.push(pool[Math.floor(Math.random() * pool.length)]);
      await env2.DB.prepare("UPDATE users SET ips = ?, last_rotate_time = ? WHERE id = ?").bind(JSON.stringify(chosen), now, u.id).run();
    }
  } catch (e) {
    console.error("autoRotateIps", e);
  }
}
__name(autoRotateIps, "autoRotateIps");
var poolSynced = false;
async function ensurePoolSynced(env2) {
  if (poolSynced)
    return;
  poolSynced = true;
  try {
    const count3 = await env2.DB.prepare("SELECT COUNT(*) AS n FROM proxies").first();
    if (count3 && count3.n > 0) {
      await syncPool(env2);
    }
  } catch (e) {
...
__name(ensurePoolSynced, "ensurePoolSynced");
async function autoResetQuotas(env2) {
  const now = Date.now();
...
__name(autoResetQuotas, "autoResetQuotas");
