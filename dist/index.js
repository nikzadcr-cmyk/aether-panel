...
`)+n.join(`
`);return{body:fr(a),contentType:"text/plain; charset=utf-8"}}function kn(e,t){let r=[],n=t.host,s=e.sni_host||n,a=e.fingerprint||"chrome",o="/"+Math.random().toString(36).slice(2,12),i=encodeURIComponent(o),c=ve(e.ips).slice(0,30),l=String(e.port||"443").split(",").map(E=>E.trim()).filter(Boolean),d=String(e.connection_type||"vless").toLowerCase(),u=d.includes("vless")||!d.includes("trojan"),h=d.includes("trojan"),p=d.includes("vmess"),f="";e.fragment&&(f+="&fragment="+encodeURIComponent(e.fragment));for(let E of c)for(let w of l){let x=En.has(w),g=x?"tls":"none",v="Aether|"+e.username+"|"+E,b=encodeURIComponent(v);if(u&&r.push("vless://"+e.uuid+"@"+E+":"+w+"?path="+i+"&security="+g+"&encryption=none&insecure=0&host="+encodeURIComponent(s)+"&fp="+a+"&type=ws&allowInsecure=0&sni="+encodeURIComponent(s)+f+"#"+b),h&&r.push("trojan://"+e.uuid+"@"+E+":"+w+"?path="+i+"&security="+g+"&insecure=0&host="+encodeURIComponent(s)+"&fp="+a+"&type=ws&allowInsecure=0&sni="+encodeURIComponent(s)+f+"#"+b),p){let k={v:"2",ps:v,add:E,port:w,id:e.uuid,aid:"0",net:"ws",type:"none",host:s,path:o,tls:x?"tls":"",sni:s};r.push("vmess://"+fr(JSON.stringify(k)))}}return r}function _n(e,t,r){let n=t.host,s=e.sni_host||n,a=ve(e.ips)[0]||oe[0],o=mr(e.route_direct),i=mr(e.route_block),c=o.map(d=>"  - DOMAIN-SUFFIX,"+d+",DIRECT").join(`
...
`).map(u=>u.trim()).filter(Boolean),l="unknown",d=[];for(let u of c)u.startsWith("#")?l=u.slice(1).trim():u.startsWith("[")||d.push(u);d.length&&(o[l]=d)}for(let i of r){let c=i.ip_operator==="all"?Object.values(o).flat():o[String(i.ip_operator||"all")]||[];if(!c.length)continue;let l=Number(i.ip_count)||15,d=[];for(let u=0;u<l&&c.length;u++)d.push(c[Math.floor(Math.random()*c.length)]);await e.DB.prepare("UPDATE users SET ips = ?, last_rotate_time = ? WHERE id = ?").bind(JSON.stringify(d),t,i.id).run()}}catch(t){console.error("autoRotateIps",t)}}var xr=!1;async function On(e){if(!xr){xr=!0;try{let t=await e.DB.prepare("SELECT COUNT(*) AS n FROM proxies").first();t&&t.n>0&&await Q(e)}catch(t){console.error("ensurePoolSynced",t)}}}async function In(e){let t=Date.now(),r=Math.floor(t/864e5)*864e5;await e.DB.prepare(`UPDATE users SET used_gb = 0, is_active = 1, last_reset_vol_time = ?
      ...
      WHERE auto_reset_req_days > 0 AND ? >= (last_reset_req_time + auto_reset_req_days * 86400000)`).bind(r,r).run()}var Eo={fetch:Pn,scheduled:Ln,queue:Bn};export{Je as PoolState,Ye as RateLimiter,Ge as UserState,Eo as default};
