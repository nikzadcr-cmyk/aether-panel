...
\u{1F517} `+i.panel+`
\u{1F464} `+(i.adminUser||"admin")+`
\u{1F511} <code>`+M(i.admin||"\u2014")+"</code>",D):await R(t,r,e.message.message_id,"\u0627\u06CC\u0646 \u062D\u0633\u0627\u0628 \u0647\u0646\u0648\u0632 \u067E\u0646\u0644\u06CC \u0646\u0633\u0627\u062E\u062A\u0647 \u0627\u0633\u062A.",Z()),new Response("ok");if(a==="list")return await q(t,e.id),await xn(t,r,e.message.message_id,i),new Response("ok")}return await q(t,e.id),new Response("ok")}async function bn(e,t,r,n){if(n==="home"){await R(e,t,r,`\u{1F3E0} <b>\u0645\u0646\u0648\u06CC \u0627\u0635\u0644\u06CC</b>
...
<\/script></body></html>`}function ur(e,t=!1){return`<!doctype html>
...
`:"")+`  - GEOIP,IR,DIRECT
  - MATCH,PROXY
`}function _n(e,t,r){let n=e.sni_host||t.host,s=Ee(e.ips)[0]||oe[0];return{log:{level:"info"},dns:{servers:[{tag:"cf",address:"https://cloudflare-dns.com/dns-query"},{tag:"local",address:"local",detour:"direct"}],rules:[{domain_suffix:[".ir"],server:"local"}]},outbounds:[{type:"vless",tag:"proxy",server:s,server_port:e.port||443,uuid:e.uuid,tls:{enabled:!0,server_name:n,utls:{enabled:!0,fingerprint:e.fingerprint||"chrome"}},transport:{type:"ws",path:"/"}},{type:"direct",tag:"direct"},{type:"block",tag:"block"}],route:{final:"proxy"}}}function hr(e){if(!e)return[];try{let t=JSON.parse(e);return Array.isArray(t)?t.filter(r=>typeof r=="string"):[]}catch{return[]}}function mr(e){let t=new TextEncoder().encode(e),r="";for(let n=0;n<t.byteLength;n++)r+=String.fromCharCode(t[n]);return btoa(r)}var An={bytesUnflushed:0,requestsUnflushed:0,lastFlush:0,active:{}},Sn=25*1024*1024,Rn=3e4,Ge=class{state;env;data={...An};flushTimer=null;constructor(t,r){this.state=t,this.env=r,this.state.blockConcurrencyWhile(async()=>{let n=await this.state.storage.get("data");n&&(this.data=n)}),this.flushTimer=setInterval(()=>{this.state.waitUntil(this.flush())},Rn)}async fetch(t){let r=new URL(t.url);switch(r.pathname.replace(/^\//,"")){case"connect":{let{ip:s,subnet:a,ua:o}=await t.json(),i=parseInt(r.searchParams.get("ipLimit")||"0",10),c=Object.keys(this.data.active);return i>0&&c.length>=i&&!this.data.active[a]?Response.json({ok:!1,code:"ip_limit",active:c.length}):(this.data.active[a]={ip:s,subnet:a,ua:o,startedAt:Date.now()},await this.persist(),Response.json({ok:!0,active:Object.keys(this.data.active).length}))}case"disconnect":{let{subnet:s}=await t.json();return delete this.data.active[s],await this.persist(),Response.json({ok:!0,active:Object.keys(this.data.active).length})}case"addBytes":{let{bytes:s,requests:a}=await t.json();return this.data.bytesUnflushed+=s|0,this.data.requestsUnflushed+=(a||0)|0,this.data.bytesUnflushed>=Sn&&this.state.waitUntil(this.flush()),Response.json({ok:!0,unflushed:this.data.bytesUnflushed})}case"status":return Response.json({active:Object.keys(this.data.active).length,unflushedBytes:this.data.bytesUnflushed,unflushedRequests:this.data.requestsUnflushed,connections:this.data.active});case"flush":return await this.flush(),Response.json({ok:!0});default:return new Response("not found",{status:404})}}async persist(){await this.state.storage.put("data",this.data)}async flush(){if(this.data.bytesUnflushed===0&&this.data.requestsUnflushed===0)return;let t=this.data.bytesUnflushed,r=this.data.requestsUnflushed;this.data.bytesUnflushed=0,this.data.requestsUnflushed=0,this.data.lastFlush=Date.now(),await this.persist();try{let n=this.state.id.name?.toString()||"";if(n){let s=t/1073741824;await this.env.DB.prepare(`UPDATE users
             SET used_gb = used_gb + ?,
                 lifetime_gb = lifetime_gb + ?,
                 used_req = used_req + ?,
                 last_active = ?
           WHERE username = ?`).bind(s,s,r,Math.floor(Date.now()/1e3),n).run()}}catch(n){this.data.bytesUnflushed+=t,this.data.requestsUnflushed+=r,await this.persist(),console.error("UserState flush failed",n)}}};var Tn={byCountry:{}},Je=class{state;env;data=Tn;alarmScheduled=!1;constructor(t,r){this.state=t,this.env=r,this.state.blockConcurrencyWhile(async()=>{let n=await this.state.storage.get("pool");n&&(this.data=n)})}async fetch(t){let r=new URL(t.url),n=r.pathname.replace(/^\//,"");if(n==="pick"){let s=(r.searchParams.get("cc")||"").toUpperCase(),o=(s?this.data.byCountry[s]||[]:Object.values(this.data.byCountry).flat()).filter(c=>c.ok);if(!o.length)return Response.json({error:"empty pool"},{status:503});let i=o[Math.floor(Math.random()*o.length)];return Response.json({uri:i.uri,latencyMs:i.latencyMs,country:i.country})}if(n==="import"){let{country:s,list:a}=await t.json(),o=(s||"XX").toUpperCase();return this.data.byCountry[o]=a.map(i=>({uri:i,country:o,ok:!0,lastChecked:0})),await this.state.storage.put("pool",this.data),this.scheduleAlarm(),Response.json({ok:!0,count:a.length})}return n==="health-check"?(Un(this.state,this.healthCheck()),Response.json({ok:!0})):new Response("not found",{status:404})}scheduleAlarm(){this.alarmScheduled||(this.alarmScheduled=!0,this.state.storage.setAlarm(6e4).catch(()=>{}))}async alarm(){this.alarmScheduled=!1,await this.healthCheck(),Object.values(this.data.byCountry).flat().length&&this.scheduleAlarm()}async healthCheck(){let r=Object.values(this.data.byCountry).flat().slice(0,50);for(let n of r){try{let s=new URL(n.uri),a=s.hostname,o=parseInt(s.port||"1080",10),i=Date.now(),c=new AbortController,l=setTimeout(()=>c.abort(),4e3),d=await fetch(`https://${a}:${o}`,{method:"HEAD",signal:c.signal,mode:"no-cors"}).catch(()=>null);clearTimeout(l),n.latencyMs=Date.now()-i,n.ok=!!d}catch{n.ok=!1}n.lastChecked=Date.now()}for(let n of Object.keys(this.data.byCountry))this.data.byCountry[n]=this.data.byCountry[n].filter(s=>s.ok);await this.state.storage.put("pool",this.data)}};function Un(e,t){try{e.waitUntil(t)}catch{}}var Ye=class{state;env;bucket={count:0,resetAt:0};constructor(t,r){this.state=t,this.env=r,this.state.blockConcurrencyWhile(async()=>{let n=await this.state.storage.get("b");n&&(this.bucket=n)})}async fetch(t){let r=new URL(t.url),n=parseInt(r.searchParams.get("limit")||"5",10),s=parseInt(r.searchParams.get("window")||"900000",10),a=Date.now();return a>this.bucket.resetAt&&(this.bucket={count:0,resetAt:a+s}),this.bucket.count+=1,await this.state.storage.put("b",this.bucket),Response.json({ok:this.bucket.count<=n,count:this.bucket.count,remaining:Math.max(0,n-this.bucket.count),resetAt:this.bucket.resetAt})}};var fr=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512">
  <defs>
...
</svg>`,gr=JSON.stringify({name:"Aether Panel",short_name:"Aether",description:"Modern Cloudflare Worker proxy panel",start_url:"/panel",scope:"/",display:"standalone",background_color:"#07090d",theme_color:"#07090d",dir:"rtl",lang:"fa-IR",orientation:"any",icons:[{src:"/icon.svg",sizes:"192x192 512x512",type:"image/svg+xml",purpose:"any maskable"}],categories:["utilities","productivity"]}),br=`
...
`).map(u=>u.trim()).filter(Boolean),l="unknown",d=[];for(let u of c)u.startsWith("#")?l=u.slice(1).trim():u.startsWith("[")||d.push(u);d.length&&(o[l]=d)}for(let i of r){let c=i.ip_operator==="all"?Object.values(o).flat():o[String(i.ip_operator||"all")]||[];if(!c.length)continue;let l=Number(i.ip_count)||15,d=[];for(let u=0;u<l&&c.length;u++)d.push(c[Math.floor(Math.random()*c.length)]);await e.DB.prepare("UPDATE users SET ips = ?, last_rotate_time = ? WHERE id = ?").bind(JSON.stringify(d),t,i.id).run()}}catch(t){console.error("autoRotateIps",t)}}var yr=!1;async function jn(e){if(!yr){yr=!0;try{let t=await e.DB.prepare("SELECT COUNT(*) AS n FROM proxies").first();t&&t.n>0&&await Q(e)}catch(t){console.error("ensurePoolSynced",t)}}}async function On(e){let t=Date.now(),r=Math.floor(t/864e5)*864e5;await e.DB.prepare(`UPDATE users SET used_gb = 0, is_active = 1, last_reset_vol_time = ?
      ...
      WHERE auto_reset_req_days > 0 AND ? >= (last_reset_req_time + auto_reset_req_days * 86400000)`).bind(r,r).run()}var wo={fetch:Cn,scheduled:Pn,queue:Ln};export{Je as PoolState,Ye as RateLimiter,Ge as UserState,wo as default};
